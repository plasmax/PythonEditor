...
class QTabBarPrivate(QWidget): # actually inherited from QWidgetPrivate

    def updateMacBorderMetrics(self):
        """
    ...
    def initBasicStyleOption(self, QStyleOptionTab *option, int tabIndex):
    ...
    def normalizedScrollRect(self, index):
    ...
    def layoutWidgets(self, start):
        Q_Q(QTabBar)
        for (int i = start; i < q.count(); ++i)
            layoutTab(i);

    def autoHideTabs(self):
        Q_Q(QTabBar)
    ...
    def moveTabFinished(self, index):
        Q_Q(QTabBar);
    ...
    def calculateNewPosition(self, _from, to, index):
        """Given that index at position _from moved to
        position to where return where index goes.
        """
        if (index == _from)
            return to;
    ...
    # #if QT_CONFIG(animation)
    # def Tab::TabBarAnimation::updateCurrentValue(const QVariant &current):

    #     priv.moveTab(priv.tabList.indexOf(*tab), current.toInt());


    # def Tab::TabBarAnimation::updateState(QAbstractAnimation::State, QAbstractAnimation::State newState):
